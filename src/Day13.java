...
public class Day13 {
    ...
    public static void main(String[] args) {
        ...
        for (List<String> pattern : patterns) {
            long horizontalSymmetryLines = countLinesAboveSymmetryLine(pattern);
            if (horizontalSymmetryLines > 0) {
                sum += (horizontalSymmetryLines * 100);
            } else {
                sum += countLinesAboveSymmetryLine(TransposeLines.execute(pattern));
            }
        }
    ...
    private static long countLinesAboveSymmetryLine(List<String> pattern) {
        long linesSum = 0;
        ...
        for (int i = 1; i < pattern.size(); i++) {
            ...
            if (presentLinesAreEqual(linesAbove, linesBelow)) {
    ...
    private static boolean presentLinesAreEqual(List<String> linesAbove, List<String> linesBelow) {
